...
// 자격증 섹션 컴포넌트 - 취득 및 취득 예정 자격증 목록 표시
export default function Certifications() {
  // 취득한 자격증 목록
  const acquiredCertifications = [
    {
      name: '네트워크 관리사 2급', // 자격증 이름
      issuer: '한국정보통신자격협회', // 발급 기관
      status: '취득', // 취득 상태
      icon: '🌐', // 아이콘
      color: 'from-green-400 to-emerald-500' // 그라데이션 색상
    }
  ]
  ...
  return (
    ...
    <section id="certifications" className="relative py-24 px-6 bg-gradient-to-b from-black via-gray-900/50 to-black">
      <div className="max-w-6xl mx-auto">
        ...
        {/* 취득한 자격증 섹션 */}
        <div className="mb-12">
          <h3 className="text-2xl font-bold text-green-400 mb-6 text-center">✓ 취득한 자격증</h3>
          <div className="grid md:grid-cols-2 gap-6">
            {acquiredCertifications.map((cert, idx) => (
              // 각 자격증 카드
              <div
                key={idx} // React key prop
                className="group relative bg-gray-900/50 backdrop-blur-sm border border-green-500/20 rounded-lg overflow-hidden hover:border-green-500/60 transition-all duration-300"
              >
                {/* 호버 시 그라데이션 배경 효과 */}
        ...
        {/* 취득 예정 자격증 섹션 */}
        <div>
          <h3 className="text-2xl font-bold text-cyan-400 mb-6 text-center">📅 취득 예정 자격증</h3>
          <div className="grid md:grid-cols-2 gap-6">
            {plannedCertifications.map((cert, idx) => (
              // 각 자격증 카드
              <div
                key={idx} // React key prop
